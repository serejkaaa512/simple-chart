pub mod graph {
    ...
    fn convert_to_display_points<'b, 'a: 'b, T>(iter: T, width: usize, height: usize) 
    ...
    where   T: 'b + Iterator<Item=&'a Point> + Clone {
        ...
        Box::new(iter.map(move |p| {
            ...
            if id_x == width {id_x -= 1;}
            if id_y == height {id_y -= 1;}
    ...
    #[test]
    fn it_works() {
        let p = vec!(Point{ x:1f64, y:1f64}, Point{ x:2f64, y:2f64}, Point{ x:3f64, y:3f64});
